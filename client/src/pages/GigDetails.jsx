  ...
  return (
    <div className="min-h-screen bg-gradient-page relative overflow-hidden page-enter" style={{ color: 'var(--color-text-primary)' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
        ...
        {!isAuthenticated && (currentGig?.status || 'open') === 'open' && (
          <div className="premium-card p-8 mb-8 text-center animate-fadeInUp" style={{ backgroundColor: 'var(--color-brand-tint)' }}>
            <div className="mb-4">
              <svg className="w-16 h-16 mx-auto animate-bounce" style={{ color: 'var(--color-brand)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <p className="mb-6" style={{ color: 'var(--color-text-secondary)' }}>
              Login or create an account to submit proposals and explore more features.
            </p>
            <div className="flex gap-3 justify-center">
              <Link
                to={`/login?redirect=${encodeURIComponent(`/gigs/${id}`)}`}
                className="premium-button"
              >
                Login to Continue
              </Link>
              <Link
                to={`/register?redirect=${encodeURIComponent(`/gigs/${id}`)}`}
